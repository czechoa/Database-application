from fastapi import Depends,FastAPI, APIRouter, HTTPException, Path, Body, Request, status
...
#     return "You password should have upper case char and digit"
#
...
@router.put("/change_password/", response_model=UserPublic, name="users:change-password")
async def change_password(
        user: UserCreateNewPassword = Body(..., embed=True),
        user_repo: UsersRepository = Depends(get_repository(UsersRepository)),
) -> UserPublic:
    user = await  user_repo.change_password(user_change_password=user)
